#!/usr/bin/env python3
"""Import ua_visual CNSF notes into Anki via AnkiConnect.
...
Usage (with Anki open + AnkiConnect running):
    ...
    python tools/anki/sync/ua_visual_import.py --dry-run domains/ua/anki/notes/visual/

    # Import a single file
    python tools/anki/sync/ua_visual_import.py domains/ua/anki/notes/visual/ua-visual-0001.md

    # Import all visual notes
    python tools/anki/sync/ua_visual_import.py domains/ua/anki/notes/visual/
"""
...
MODEL_NAME = "UA_Visual"
DECK_VISUAL = "UA::Recognition::Visual"
...
def import_note(data: dict, dry_run: bool) -> str:
    ...
    # Coerce all field values to strings
...
def collect_files(targets: list[str]) -> list[Path]:
    ...
    for t in targets:
        p = Path(t)
        if p.is_dir():
            files.extend(sorted(p.glob("ua-visual-*.md")))
        elif p.is_file():
            files.append(p)
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Import ua_visual notes into Anki.")
    ...
    ensure_deck(DECK_VISUAL, args.dry_run)

    files = collect_files(args.targets)
    if not files:
        print("No ua-visual-*.md files found.")
        sys.exit(1)
    ...
    for f in files:
        ...
        try:
            ...
            prefix = (data.get("fields") or {}).get("Prefix", "")
            label = f"{note_id}  {prefix}"
            if result == "added":
                print(f"  ADD     {label}")
                added += 1
            elif result == "updated":
