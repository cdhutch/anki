#!/usr/bin/env python3
"""Import ua_grammar CNSF notes into Anki via AnkiConnect.
...
Usage (with Anki open + AnkiConnect running):
    ...
    python tools/anki/sync/ua_grammar_import.py domains/ua/anki/notes/grammar/ua-grammar-0001.md

    # Import all grammar notes
    python tools/anki/sync/ua_grammar_import.py domains/ua/anki/notes/grammar/
"""
...
def collect_files(targets: list[str]) -> list[Path]:
    ...
    for t in targets:
        p = Path(t)
        if p.is_dir():
            files.extend(sorted(p.glob("ua-grammar-*.md")))
        elif p.is_file():
            files.append(p)
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Import ua_grammar notes into Anki.")
    ...
    ensure_deck(DECK_GRAMMAR, args.dry_run)
