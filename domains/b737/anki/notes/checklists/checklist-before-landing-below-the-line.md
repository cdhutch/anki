---
schema: cnsf/v0
domain: b737
note_type: checklist
note_id: checklist-before-landing-below-the-line
anki:
  model: B737_Checklist
  deck: B737::Core::Checklists
tags:
- domain:b737
- topic:procedures
- type:checklist
- checklist:before_landing
- status:unverified
fields:
  Checklist Name: Before Landing Checklist (Below The Line)
  Source Document: American Airlines B737 Aircraft Operating Manual Revision Number 9.0
  Verification Notes: ''
---

# front_md

| Challenge |
|---|
| SPEEDBRAKES |
| GEAR |
| FLAPS |

# back_md

<table>
<thead><tr><th>Challenge</th><th>Response</th></tr></thead>
<tbody>
<tr style="background-color:#d6e4f7;"><td>SPEEDBRAKES</td><td>ARMED, Green Light ☨</td></tr>
<tr style="background-color:#d6e4f7;"><td>GEAR</td><td>DOWN and Green ☨</td></tr>
<tr style="background-color:#d6e4f7;"><td>FLAPS</td><td>___, Green Light ☨</td></tr>
</tbody>
</table>
