---
schema: cnsf/v0
note_type: ipa_phoneme
note_id: ipa-phoneme-0070
anki:
  model: IPA_Phoneme
  deck: IPA::Vowels
tags:
- status:draft
fields:
  NoteID: ipa-phoneme-0070
  Phoneme: near-close-near-back-rounded-vowel
  IPA_Symbol: ʊ
  Type: vowel
  Description: Near-close near-back rounded vowel
  Manner_of_Articulation: ''
  Place_of_Articulation: ''
  Voicing: ''
  Airflow: ''
  Example_Words: <div class="ipa-example-line"><span class="ipa-lang">English</span>b<b>oo</b>k</div><div
    class="ipa-example-line"><span class="ipa-lang">German</span>M<b>u</b>tter (mother)</div>
  Language_Analogs: ''
  Minimal_Pairs: ''
  Confusable_With: ''
  Mnemonic_EN: ''
  EN_Gloss: ''
  Tags_Ch: ''
  Source_URL: ''
  Source_Note: Cross-checked against Wikipedia's English_phonology, Standard_German_phonology,
    French_phonology, Russian_phonology, and Ukrainian_phonology articles; see round1_phoneme_card.md
    methodology notes for scope decisions (length, diphthongs, palatalization).
  Verification Notes: ''
---

