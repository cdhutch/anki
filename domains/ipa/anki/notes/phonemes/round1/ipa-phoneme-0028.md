---
schema: cnsf/v0
note_type: ipa_phoneme
note_id: ipa-phoneme-0028
anki:
  model: IPA_Phoneme
  deck: IPA::Consonants
tags:
- status:draft
fields:
  NoteID: ipa-phoneme-0028
  Phoneme: voiced-alveolar-trill
  IPA_Symbol: r
  Type: consonant
  Description: Voiced alveolar trill
  Manner_of_Articulation: trill
  Place_of_Articulation: alveolar
  Voicing: voiced
  Airflow: ''
  Example_Words: <div class="ipa-example-line"><span class="ipa-lang">Russian</span><b>р</b>ука
    (hand)</div><div class="ipa-example-line"><span class="ipa-lang">Ukrainian</span><b>р</b>ука
    (hand)</div>
  Language_Analogs: ''
  Minimal_Pairs: ''
  Confusable_With: ''
  Mnemonic_EN: ''
  EN_Gloss: ''
  Tags_Ch: ''
  Source_URL: ''
  Source_Note: Cross-checked against Wikipedia's English_phonology, Standard_German_phonology,
    French_phonology, Russian_phonology, and Ukrainian_phonology articles; see round1_phoneme_card.md
    methodology notes for scope decisions (length, diphthongs, palatalization).
  Verification Notes: ''
---

