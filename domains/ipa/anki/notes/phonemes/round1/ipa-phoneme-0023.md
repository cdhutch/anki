---
schema: cnsf/v0
note_type: ipa_phoneme
note_id: ipa-phoneme-0023
anki:
  model: IPA_Phoneme
  deck: IPA::Consonants
tags:
- status:draft
fields:
  NoteID: ipa-phoneme-0023
  Phoneme: voiceless-glottal-fricative
  IPA_Symbol: h
  Type: consonant
  Description: Voiceless glottal fricative
  Manner_of_Articulation: fricative
  Place_of_Articulation: glottal
  Voicing: voiceless
  Airflow: ''
  Example_Words: <div class="ipa-example-line"><span class="ipa-lang">English</span><b>h</b>at</div><div
    class="ipa-example-line"><span class="ipa-lang">German</span><b>H</b>aus (house)</div>
  Language_Analogs: ''
  Minimal_Pairs: ''
  Confusable_With: ''
  Mnemonic_EN: ''
  EN_Gloss: ''
  Tags_Ch: ''
  Source_URL: ''
  Source_Note: Cross-checked against Wikipedia's English_phonology, Standard_German_phonology,
    French_phonology, Russian_phonology, and Ukrainian_phonology articles; see round1_phoneme_card.md
    methodology notes for scope decisions (length, diphthongs, palatalization).
  Verification Notes: ''
---

