---
schema: cnsf/v0
note_type: ipa_phoneme
note_id: ipa-phoneme-0033
anki:
  model: IPA_Phoneme
  deck: IPA::Consonants
tags:
- status:draft
fields:
  NoteID: ipa-phoneme-0033
  Phoneme: voiceless-alveolar-affricate
  IPA_Symbol: ts
  Type: consonant
  Description: Voiceless alveolar affricate
  Manner_of_Articulation: affricate
  Place_of_Articulation: alveolar
  Voicing: voiceless
  Airflow: ''
  Example_Words: <div class="ipa-example-line"><span class="ipa-flag">🇩🇪</span><b class="ipa-sound">Z</b>eit
    (time)</div><div class="ipa-example-line"><span class="ipa-flag">🇷🇺</span>яй<b class="ipa-sound">ц</b>о
    (egg)</div><div class="ipa-example-line"><span class="ipa-flag">🇺🇦</span>яй<b class="ipa-sound">ц</b>е
    (egg)</div>
  Language_Analogs: ''
  Minimal_Pairs: ''
  Confusable_With: ''
  Mnemonic_EN: ''
  EN_Gloss: ''
  Tags_Ch: ''
  Source_URL: ''
  Source_Note: Cross-checked against Wikipedia's English_phonology, Standard_German_phonology,
    French_phonology, Russian_phonology, and Ukrainian_phonology articles; see round1_phoneme_card.md
    methodology notes for scope decisions (length, diphthongs, palatalization).
  Verification Notes: ''
---

