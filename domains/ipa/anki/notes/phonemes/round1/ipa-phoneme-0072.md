---
schema: cnsf/v0
note_type: ipa_phoneme
note_id: ipa-phoneme-0072
anki:
  model: IPA_Phoneme
  deck: IPA::Vowels
tags:
- status:draft
fields:
  NoteID: ipa-phoneme-0072
  Phoneme: mid-central-vowel-schwa
  IPA_Symbol: ə
  Type: vowel
  Description: Mid central vowel (schwa)
  Manner_of_Articulation: ''
  Place_of_Articulation: ''
  Voicing: ''
  Airflow: ''
  Example_Words: <div class="ipa-example-line"><span class="ipa-flag">🇺🇸</span><b>a</b>bout</div><div
    class="ipa-example-line"><span class="ipa-flag">🇩🇪</span>b<b>i</b>tt<b>e</b> (please)</div><div
    class="ipa-example-line"><span class="ipa-flag">🇫🇷</span><b>le</b> (the)</div>
  Language_Analogs: ''
  Minimal_Pairs: ''
  Confusable_With: ''
  Mnemonic_EN: ''
  EN_Gloss: ''
  Tags_Ch: ''
  Source_URL: ''
  Source_Note: Cross-checked against Wikipedia's English_phonology, Standard_German_phonology,
    French_phonology, Russian_phonology, and Ukrainian_phonology articles; see round1_phoneme_card.md
    methodology notes for scope decisions (length, diphthongs, palatalization).
  Verification Notes: ''
---

