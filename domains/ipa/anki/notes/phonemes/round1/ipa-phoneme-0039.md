---
schema: cnsf/v0
note_type: ipa_phoneme
note_id: ipa-phoneme-0039
anki:
  model: IPA_Phoneme
  deck: IPA::Consonants
tags:
- status:draft
fields:
  NoteID: ipa-phoneme-0039
  Phoneme: voiceless-labial-affricate-german
  IPA_Symbol: pf
  Type: consonant
  Description: Voiceless labial affricate (German)
  Manner_of_Articulation: affricate
  Place_of_Articulation: bilabial-labiodental
  Voicing: voiceless
  Airflow: ''
  Example_Words: <div class="ipa-example-line"><span class="ipa-flag">🇩🇪</span><b class="ipa-sound">Pf</b>effer
    (pepper)</div>
  Language_Analogs: ''
  Minimal_Pairs: ''
  Confusable_With: ''
  Mnemonic_EN: ''
  EN_Gloss: ''
  Tags_Ch: ''
  Source_URL: ''
  Source_Note: Cross-checked against Wikipedia's English_phonology, Standard_German_phonology,
    French_phonology, Russian_phonology, and Ukrainian_phonology articles; see round1_phoneme_card.md
    methodology notes for scope decisions (length, diphthongs, palatalization).
  Verification Notes: ''
---

