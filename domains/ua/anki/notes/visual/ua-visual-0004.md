---
schema: cnsf/v0
note_type: ua_visual
note_id: ua-visual-0004
anki:
  model: UA_Visual
  deck: UA::Recognition::Visual
tags:
- domain:ua
- topic:grammar
- textbook:яблуко
- ch:2.9.4
- grammar:verb_motion
- grammar:prefix
- status:verified
fields:
  NoteID: ua-visual-0004
  Prefix: під-
  Meaning_EN: approaching
  Govt: до + Р.в.
  Walking_Pair: підхо́дити / підійти́
  Vehicle_Pair: підʼїжджа́ти / підʼї́хати
  Example_UA: Він підійшов до вікна.
  Example_EN: He approached the window.
  Diagram_SVG: '<svg viewBox="0 0 260 120" xmlns="http://www.w3.org/2000/svg" style="max-width:260px;display:block;margin:0
    auto"><defs><marker id="arr" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path
    d="M0,0 L0,6 L8,3 z" fill="#859900"/></marker></defs><!-- Legend --><text x="5" y="12"
    font-size="10" font-weight="bold" fill="#586e75">Legend:</text><circle cx="10" cy="25"
    r="4" fill="#dc322f"/><text x="20" y="28" font-size="9" fill="#586e75">Location</text><!--
    Main: arrow stops before reaching the point --><circle cx="30" cy="75" r="5" fill="#268bd2"/><line
    x1="40" y1="75" x2="195" y2="75" stroke="#859900" stroke-width="2.5" marker-end="url(#arr)"/><line
    x1="195" y1="60" x2="195" y2="90" stroke="#93a1a1" stroke-width="1.5" stroke-dasharray="3,3"/><circle
    cx="220" cy="75" r="6" fill="#dc322f"/></svg>'
  Tags_Ch: ch:2.9.4
  Source_Note: під- — approach (stop before reaching)
  Verification Notes: ''
---

