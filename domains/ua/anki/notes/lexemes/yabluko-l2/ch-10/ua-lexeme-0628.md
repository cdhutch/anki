---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0628
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.10.1
- pos:noun
- status:draft
- ch:2.1
fields:
  NoteID: ua-lexeme-0628
  Lemma: магази́н
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: m
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: shop, store
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: магазин
  UA_Example: У аеропорту є магазин безмитної торгівлі.
  EN_Example: There's a duty-free shop at the airport.
  Tags_Ch: ch:2.10.1, ch:2.1
  Source_URL: https://goroh.pp.ua/Словозміна/магазин
  Source_Note: Горох-verified 2026-08-28, ch.10.1 sourcing pass (airport/travel prohibitions).
  Verification Notes: 'Горох lists two homograph senses sharing one spelling/stress: (1) ''пристрій''
    (a device -- e.g. a gun''s magazine), (2) ''крамниця'' (shop/store). Only sense 2 is drafted
    here (the ch.2/ch.10 ''shop'' vocabulary sense); the two senses differ in genitive singular
    (магази́на for sense 1''s device vs магази́ну for sense 2''s shop) if sense 1 is ever
    needed. Not tagged homograph:true -- no sibling note exists for sense 1.'
---

