---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0771
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.10.6
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-0771
  Lemma: огля́нути
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to examine (medically); to inspect, to look over
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: оглянути
  UA_Example: Лікар оглянув пацієнта і призначив лікування.
  EN_Example: The doctor examined the patient and prescribed treatment.
  Tags_Ch: ch:2.10.6
  Source_URL: https://goroh.pp.ua/Словозміна/оглянути
  Source_Note: Горох-verified 2026-08-28, ch.10.6 sourcing pass (illness/injury symptoms;
    body parts; treatment).
  Verification Notes: New. Perfective. Imperfective counterpart оглядати not drafted this
    pass. Full conjugation on paired ua-verb-0123. Imperfective counterpart огляда́ти added
    (aspect-pairing review, yabluko-l2-verb-dictionary + Горох, 2026-08-28).
---

