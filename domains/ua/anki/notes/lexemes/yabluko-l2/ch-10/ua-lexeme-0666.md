---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0666
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.10.2
- ch:1.11.2
- ch:1.12.2
- pos:verb
- status:draft
- release:active
- relearn:seeded
fields:
  NoteID: ua-lexeme-0666
  Lemma: дзвони́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: подзвони́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to call (by phone), to ring
  Govt_Case: Dative
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: дзвонити / подзвонити
  UA_Example: Я подзвоню тобі ввечері.
  EN_Example: I'll call you in the evening.
  Tags_Ch: ch:2.10.2, ch:1.11.2, ch:1.12.2
  Source_URL: https://goroh.pp.ua/Словозміна/дзвонити; https://goroh.pp.ua/Словозміна/подзвонити
  Source_Note: Горох-verified 2026-08-28, ch.10.2 sourcing pass (family/games; feelings &
    social verbs).
  Verification Notes: 'New. Imperfective, non-reflexive, 2nd conjugation. Governs dative:
    дзвонити комусь. Perfective подзвони́ти (Горох-confirmed). Full conjugation on the paired
    ua-verb-0094 note.'
---

