---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0825
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.10.7
- pos:phrase
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-0825
  Lemma: зверну́тися до лі́каря
  Lemma_Euphony: ''
  PartOfSpeech: phrase
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to see / consult a doctor
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: звернутися до лікаря
  UA_Example: Якщо симптоми не минають, звернися до лікаря.
  EN_Example: If the symptoms don't go away, see a doctor.
  Tags_Ch: ch:2.10.7
  Source_URL: https://goroh.pp.ua/Словозміна/звернутися
  Source_Note: Горох-verified 2026-08-28, ch.10.7 sourcing pass (consular services; border/customs;
    travel health).
  Verification Notes: References звернутися (ua-lexeme-1147, verb) and the pre-existing лікар
    (ua-lexeme-0006, ch:1.0 -- referenced as a component, not re-tagged, same precedent as
    кинути палити in ch.10.3).
---

