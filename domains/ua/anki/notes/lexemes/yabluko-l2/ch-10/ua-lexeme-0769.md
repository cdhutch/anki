---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0769
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.10.6
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-0769
  Lemma: ви́писати
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to write out, to issue (a prescription); to discharge (from hospital)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: виписати
  UA_Example: Лікар виписав мені рецепт на антибіотики.
  EN_Example: The doctor wrote me a prescription for antibiotics.
  Tags_Ch: ch:2.10.6
  Source_URL: https://goroh.pp.ua/Словозміна/виписати
  Source_Note: Горох-verified 2026-08-28, ch.10.6 sourcing pass (illness/injury symptoms;
    body parts; treatment).
  Verification Notes: New. Perfective. No imperfective counterpart (виписувати) drafted this
    pass. Full conjugation on paired ua-verb-0121. Imperfective counterpart випи́сувати added
    (aspect-pairing review, yabluko-l2-verb-dictionary + Горох, 2026-08-28).
---

