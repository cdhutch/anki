---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0620
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.7.3
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.10.1
- pos:noun
- status:verified
- release:active
- relearn:pending
fields:
  NoteID: ua-lexeme-0620
  Lemma: валі́за
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: suitcase
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: cf. поклажа (the abstract, collective "baggage/load," not any one item),
    сумка, мішок, торба (other bag/sack types) -- валіза is specifically a suitcase.
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: валіза
  UA_Example: Моя валіза важить двадцять кілограмів.
  EN_Example: My suitcase weighs twenty kilograms.
  Tags_Ch: ch:2.10.1, ch:1.7.3
  Source_URL: https://goroh.pp.ua/Словозміна/валіза
  Source_Note: Горох-verified 2026-08-28, ch.10.1 sourcing pass (airport/travel prohibitions).
  Verification Notes: New. Fem., 1st decl., hard group.
---

