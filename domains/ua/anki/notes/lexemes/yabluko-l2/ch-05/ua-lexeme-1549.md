---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1549
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.7.2
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.5.2
- ch:1.12.5
- pos:noun
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1549
  Lemma: підло́га
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: floor
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: підлога
  UA_Example: На підлозі лежав старий килим.
  EN_Example: An old rug lay on the floor.
  Tags_Ch: ch:2.5.2, ch:1.7.2, ch:1.12.5
  Source_URL: https://goroh.pp.ua/Словозміна/підлога
  Source_Note: Горох-verified 2026-08-28, ch.5.2 sourcing pass (train travel/station; house
    features; sightseeing verbs and adjectives).
  Verification Notes: New. Fem., 1st decl., hard group. Stress підло́га (2nd syllable) --
    corrected from a garbled Горох summarizer reading that asserted first-syllable stress
    in its prose while its own rendered form showed the accent on the second syllable.
---

