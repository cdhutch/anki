---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1919
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.2
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1919
  Lemma: обмі́нюватися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to exchange (with each other) (imperfective)
  Govt_Case: Ор.
  IrregularForms: ''
  CounterpartForm: обміня́тися (ua-lexeme-1920, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: обмінюватися
  UA_Example: Друзі обмінюються паролями від ігор.
  EN_Example: The friends exchange passwords for games.
  Tags_Ch: ch:2.7.2
  Source_URL: https://goroh.pp.ua/Словозміна/обмінюватися
  Source_Note: Горох-verified 2026-08-28, ch.7.2 sourcing pass (accounts/passwords tech vocabulary;
    frequency adverbs; descriptive adjectives).
  Verification Notes: New. Imperfective, "to exchange", reciprocal reflexive verb governing
    the instrumental case. Textbook aspect pair обмінюватися-обмінятися, this sub-chapter.
---

