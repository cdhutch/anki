---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1882
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.1
- ch:1.12.2
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-1882
  Lemma: раху́нок
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: m
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: account; bill, invoice
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: рахунок
  UA_Example: Він забув поповнити рахунок на телефоні.
  EN_Example: He forgot to top up the balance on his phone account.
  Tags_Ch: ch:2.7.1, ch:1.12.2
  Source_URL: https://goroh.pp.ua/Словозміна/рахунок
  Source_Note: Горох-verified 2026-08-28, ch.7.1 sourcing pass (devices/tech vocabulary).
  Verification Notes: New. Masc., 2nd decl., hard group, fill-vowel (о) in the oblique stem
    (раху́нку/раху́нка). Drafted for the account-balance sense (matches поповнювати - поповнити
    (рахунок), this sub-chapter).
---

