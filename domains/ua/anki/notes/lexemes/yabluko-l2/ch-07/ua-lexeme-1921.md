---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1921
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.2
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1921
  Lemma: дізнава́тися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to find out, learn (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: дізна́тися (ua-lexeme-1922, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: дізнаватися
  UA_Example: Як я можу дізнаватися про нові оновлення сервісу?
  EN_Example: How can I find out about new updates to the service?
  Tags_Ch: ch:2.7.2
  Source_URL: https://goroh.pp.ua/Словозміна/дізнаватися
  Source_Note: Горох-verified 2026-08-28, ch.7.2 sourcing pass (accounts/passwords tech vocabulary;
    frequency adverbs; descriptive adjectives).
  Verification Notes: New. Imperfective, "to find out, learn", -авати-type theme (drops -ва-
    in the non-past, давати-family pattern). Textbook aspect pair дізнаватися-дізнатися, this
    sub-chapter.
---

