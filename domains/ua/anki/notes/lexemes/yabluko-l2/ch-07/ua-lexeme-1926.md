---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1926
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.2
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1926
  Lemma: прода́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to sell (perfective)
  Govt_Case: ''
  IrregularForms: 'Suppletive non-past paradigm (дати-family: продам, продаси, продасть, продамо,
    продасте, продадуть).'
  CounterpartForm: продава́ти (ua-lexeme-1925, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: продати
  UA_Example: Він продав старий телефон і купив новий.
  EN_Example: He sold his old phone and bought a new one.
  Tags_Ch: ch:2.7.2
  Source_URL: https://goroh.pp.ua/Словозміна/продати
  Source_Note: Горох-verified 2026-08-28, ch.7.2 sourcing pass (accounts/passwords tech vocabulary;
    frequency adverbs; descriptive adjectives).
  Verification Notes: New. Perfective partner of продава́ти (ua-lexeme-1925), suppletive дати-family
    conjugation.
---

