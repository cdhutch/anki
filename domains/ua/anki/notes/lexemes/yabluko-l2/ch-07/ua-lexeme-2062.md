---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2062
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.7
- ch:1.11.6
- ch:1.12.5
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-2062
  Lemma: дозво́лити
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to allow, to permit (perfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: дозволя́ти (ua-lexeme-2061, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: дозволити
  UA_Example: Вчитель дозволив учням піти раніше.
  EN_Example: The teacher allowed the students to leave early.
  Tags_Ch: ch:2.7.7, ch:1.11.6, ch:1.12.5
  Source_URL: https://goroh.pp.ua/Словозміна/дозволити
  Source_Note: Горох-verified 2026-08-28, ch.7.7 sourcing pass (consumption/ownership nouns;
    test/allow/arrange/save/build verb pairs; connector adverbs) -- final sub-chapter of chapter
    7 (Наше майбутнє).
  Verification Notes: New. Perfective partner of дозволя́ти (ua-lexeme-2061), fixed-stress
    conj2-ity.
---

