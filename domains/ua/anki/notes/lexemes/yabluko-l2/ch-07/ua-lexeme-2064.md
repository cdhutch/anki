---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2064
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.7
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-2064
  Lemma: домо́витися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to arrange, to agree (perfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: домовля́тися (ua-lexeme-2063, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: домовитися
  UA_Example: Вони домовилися зустрітися о шостій.
  EN_Example: They agreed to meet at six.
  Tags_Ch: ch:2.7.7
  Source_URL: https://goroh.pp.ua/Словозміна/домовитися
  Source_Note: Горох-verified 2026-08-28, ch.7.7 sourcing pass (consumption/ownership nouns;
    test/allow/arrange/save/build verb pairs; connector adverbs) -- final sub-chapter of chapter
    7 (Наше майбутнє).
  Verification Notes: New. Perfective partner of домовля́тися (ua-lexeme-2063), fixed-stress
    conj2-ity reflexive with epenthetic -л- (в→вл) in 1sg/3pl.
---

