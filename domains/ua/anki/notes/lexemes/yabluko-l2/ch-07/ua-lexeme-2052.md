---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2052
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.6
- pos:phrase
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-2052
  Lemma: Ви́бач, але я не мо́жу. Мо́же, і́ншим ра́зом?
  Lemma_Euphony: ''
  PartOfSpeech: phrase
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: Sorry, I can't. Maybe another time?
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: Вибач, але я не можу. Може, іншим разом?
  UA_Example: Вибач, але я не можу. Може, іншим разом?
  EN_Example: Sorry, I can't. Maybe another time?
  Tags_Ch: ch:2.7.6
  Source_URL: https://goroh.pp.ua/Словозміна/раз
  Source_Note: Горох-verified 2026-08-28, ch.7.6 sourcing pass (virtual reality; coordinate/gather/decline
    verb pairs; invitation discourse formulas).
  Verification Notes: New. Decline discourse formula, one phrase note.
---

