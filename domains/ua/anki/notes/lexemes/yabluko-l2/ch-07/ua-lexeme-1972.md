---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1972
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7.3
- pos:verb
- status:draft
- release:pending
- ch:2.12.6
fields:
  NoteID: ua-lexeme-1972
  Lemma: передба́чити
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to predict, to foresee (perfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: передбача́ти (ua-lexeme-1971, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: передбачити
  UA_Example: Науковці передбачили появу цієї технології ще 20 років тому.
  EN_Example: Scientists predicted the appearance of this technology 20 years ago already.
  Tags_Ch: ch:2.7.3,ch:2.12.6
  Source_URL: https://goroh.pp.ua/Словозміна/передбачити
  Source_Note: Горох-verified 2026-08-28, ch.7.3 sourcing pass (future tech/robotics/medicine
    nouns and adjectives; a large, page-spanning sub-chapter per the two-independent-columns
    PDF layout discovery).
  Verification Notes: New. Perfective partner of передбача́ти (ua-lexeme-1971), fixed-stress
    conj2-ity (matches бачити's paradigm).
---

