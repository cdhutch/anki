---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2255
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.9.1
- ch:1.7.7
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.12.7
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-2255
  Lemma: осо́ба
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f., 1st decl., hard group
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: 'person (grammatical term; also: person, individual)'
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: особа
  UA_Example: Дієслово тут стоїть у третій особі.
  EN_Example: The verb here is in the third person.
  Tags_Ch: ch:2.12.7, ch:1.7.7, ch:1.9.1
  Source_URL: https://goroh.pp.ua/Словозміна/особа
  Source_Note: Горох-verified 2026-08-29, ch.12.7 sourcing pass (grammar terminology; pronunciation/dictation
    verbs; suffering verbs; manner adverbs), cross-checked against yabluko-l2-verb-dictionary.pdf
    for every verb's aspect partner per Craig's explicit instruction. Confirmed via direct
    visual read of PDF page 223 (Read tool, image mode).
  Verification Notes: New. Fem., 1st decl., hard group. Drafted in the grammatical-person
    sense per this subchapter's terminology list; the general 'person, individual' sense is
    the same word and not a separate homograph.
---

