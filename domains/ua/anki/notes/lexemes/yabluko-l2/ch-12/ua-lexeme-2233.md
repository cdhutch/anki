---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2233
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.12.5
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-2233
  Lemma: надиха́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: надихну́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to inspire (imperfective)
  Govt_Case: Зн., Ор.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: надихати / надихнути
  UA_Example: Її історія надихає багатьох людей.
  EN_Example: Her story inspires many people.
  Tags_Ch: ch:2.12.5
  Source_URL: https://goroh.pp.ua/Словозміна/надихати
  Source_Note: 'Горох-verified 2026-08-29, ch.12.5 sourcing pass (community/history & character
    vocabulary: ancestry, village life, civic/political terms, personality traits), cross-checked
    against yabluko-l2-verb-dictionary.pdf for every verb''s aspect partner per Craig''s explicit
    instruction. Confirmed via direct visual read of PDF page 223 (Read tool, image mode).'
  Verification Notes: 'New. impf., non-reflexive, regular conj1-vowel+й. Note: Горох''s own
    page for the bare spelling ''надихати'' is a homograph-by-stress pair (root-stressed надиха́ти
    = this regular imperfective; a separately-listed prefix-stressed нади́хати is a rare perfective
    variant), parallel to the ви́бухати/вибуха́ти and ви́писати/виписувати stress patterns
    already documented in this corpus. Perfective надихнути confirmed via yabluko-l2-verb-dictionary.pdf
    (line 913: ''надихати надихнути Зн, Ор.''). Merged duplicate standalone perfective note
    ua-lexeme-2233 (aspect-pairing dual-convention cleanup, 2026-08-29). That sub-chapter''s
    example: ''Ця книга надихнула мене на подорож.'' = ''This book inspired me to travel.''.'
---

