---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2270
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.12.7
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-2270
  Lemma: му́читися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: пому́читися
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to suffer, agonize, torment oneself (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: мучитися / помучитися
  UA_Example: Вона довго мучилася над цим рішенням.
  EN_Example: She agonized over this decision for a long time.
  Tags_Ch: ch:2.12.7
  Source_URL: https://goroh.pp.ua/Словозміна/мучитися
  Source_Note: Горох-verified 2026-08-29, ch.12.7 sourcing pass (grammar terminology; pronunciation/dictation
    verbs; suffering verbs; manner adverbs), cross-checked against yabluko-l2-verb-dictionary.pdf
    for every verb's aspect partner per Craig's explicit instruction. Confirmed via direct
    visual read of PDF page 223 (Read tool, image mode).
  Verification Notes: 'New. impf., reflexive, conj2-ity (regular, no consonant mutation: stem-final
    ч already present). Not listed in yabluko-l2-verb-dictionary.pdf and Горох''s own page
    for мучитися lists no perfective -- however, помучитися is independently attested on Горох
    as a standard, fully-conjugated perfective (delimitative ''to suffer for a while''), so
    it is added here as the perfective partner per Craig''s instruction rather than treating
    мучитися as single-aspect.'
---

