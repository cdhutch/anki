---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2195
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.12.3
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-2195
  Lemma: адапта́ція
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f., 1st decl., mixed group (-ія)
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: adaptation
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: адаптація
  UA_Example: Адаптація до нового середовища часто триває довго.
  EN_Example: Adaptation to a new environment often takes a long time.
  Tags_Ch: ch:2.12.3
  Source_URL: https://goroh.pp.ua/Словозміна/адаптація
  Source_Note: Горох-verified 2026-08-29, ch.12.3 sourcing pass (migration/adaptation vocabulary;
    settling-in and worry/anxiety verbs), cross-checked against yabluko-l2-verb-dictionary.pdf
    for every verb's aspect partner per Craig's explicit instruction. Confirmed via direct
    visual read of PDF page 222 (Read tool, image mode).
  Verification Notes: New. Fem., 1st decl., mixed group (-ія).
---

