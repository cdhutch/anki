---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2268
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.12.7
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-2268
  Lemma: диктува́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: продиктува́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to dictate (imperfective)
  Govt_Case: Зн., Д.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: диктувати / продиктувати
  UA_Example: Учителька диктує текст учням по літерах.
  EN_Example: The teacher dictates the text to the students letter by letter.
  Tags_Ch: ch:2.12.7
  Source_URL: https://goroh.pp.ua/Словозміна/диктувати
  Source_Note: Горох-verified 2026-08-29, ch.12.7 sourcing pass (grammar terminology; pronunciation/dictation
    verbs; suffering verbs; manner adverbs), cross-checked against yabluko-l2-verb-dictionary.pdf
    for every verb's aspect partner per Craig's explicit instruction. Confirmed via direct
    visual read of PDF page 223 (Read tool, image mode).
  Verification Notes: 'New. impf., non-reflexive, regular conj1-vowel+й (-увати class). Sense
    note: textbook bullets this as the collocation ''диктувати по літерах'' = ''to spell out/dictate
    letter by letter'' (літера drafted standalone, ua-lexeme-2262). Perfective продиктувати
    confirmed via yabluko-l2-verb-dictionary.pdf (line 439: ''диктувати продиктувати Зн.,
    Д.''). Merged duplicate standalone perfective note ua-lexeme-2268 (aspect-pairing dual-convention
    cleanup, 2026-08-29). That sub-chapter''s example: ''Продиктуйте мені, будь ласка, свою
    адресу.'' = ''Please dictate your address to me.''.'
---

