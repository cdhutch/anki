---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1166
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.7.5
- ch:1.7.3
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.3.1
- pos:noun
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1166
  Lemma: скля́нка
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: glass (drinking vessel)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: Горох also lists скля́нка "проміжок часу" (a historical/nautical time-unit,
    "a bell" of a watch) under the same spelling and declension; not registered (the drinking-vessel
    sense is meant here).
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: склянка
  UA_Example: Випий склянку води.
  EN_Example: Drink a glass of water.
  Tags_Ch: ch:2.3.1, ch:1.7.3, ch:1.7.5
  Source_URL: https://goroh.pp.ua/Словозміна/склянка
  Source_Note: Горох-verified 2026-08-28, ch.3.1 sourcing pass (quantities/containers).
  Verification Notes: New. Fem., 1st decl., hard group.
---

