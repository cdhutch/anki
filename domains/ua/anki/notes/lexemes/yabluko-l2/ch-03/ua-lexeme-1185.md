---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1185
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:3.3
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-1185
  Lemma: гли́на
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: clay
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: Горох алсо lists Глина (capitalized) as a river name in Bosnia/Croatia, an
    unrelated toponym; not registered.
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: глина
  UA_Example: Дитина ліпить з глини.
  EN_Example: The child is molding with clay.
  Tags_Ch: ch:3.3
  Source_URL: https://goroh.pp.ua/Словозміна/глина
  Source_Note: Горох-verified 2026-08-28, ch.3.3 sourcing pass (eco-village/village-life).
  Verification Notes: New. Fem., 1st decl., hard group.
---

