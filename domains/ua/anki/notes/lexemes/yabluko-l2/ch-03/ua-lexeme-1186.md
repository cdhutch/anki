---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1186
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:3.3
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-1186
  Lemma: доро́га
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: road
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: Горох алсо lists дороги́й "expensive; dear" (adjective, different stress)
    under a shared spelling family; unrelated word, not registered.
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: дорога
  UA_Example: Дорога до екопоселення веде через ліс.
  EN_Example: The road to the eco-village leads through the forest.
  Tags_Ch: ch:3.3
  Source_URL: https://goroh.pp.ua/Словозміна/дорога
  Source_Note: Горох-verified 2026-08-28, ch.3.3 sourcing pass (eco-village/village-life).
  Verification Notes: New. Fem., 1st decl., hard group.
---

