---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1303
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.3.6
- ch:1.5.3
- ch:1.6.1
- pos:noun
- gender:f
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1303
  Lemma: заку́ска
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: appetizer, starter (food)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: закуска
  UA_Example: На закуску ми замовили брускету.
  EN_Example: For a starter, we ordered bruschetta.
  Tags_Ch: ch:2.3.6, ch:1.5.3, ch:1.6.1
  Source_URL: https://goroh.pp.ua/Словозміна/закуска
  Source_Note: 'Горох-verified 2026-08-28, ch.3.6 sourcing pass (restaurant/cafe: menu items,
    dishes, ordering formulas).'
  Verification Notes: New. Fem., 1st decl., hard group. Wordlist bullet is plural "закуски"
    (menu-category usage); drafted as the singular dictionary lemma.
---

