---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1272
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.3.4
- ch:1.2.5
- ch:1.3.3
- ch:1.5.4
- pos:adverb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1272
  Lemma: лише́
  Lemma_Euphony: ''
  PartOfSpeech: adverb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: only, just
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: лише
  UA_Example: У опитуванні взяли участь лише кілька мешканців.
  EN_Example: Only a few residents took part in the survey.
  Tags_Ch: ch:2.3.4, ch:1.2.5, ch:1.3.3, ch:1.5.4
  Source_URL: https://goroh.pp.ua/Словозміна/лише
  Source_Note: Горох-verified 2026-08-28, ch.3.4 sourcing pass (village/town governance, heritage
    architecture, public opinion).
  Verification Notes: New. Invariable; Горох also lists it as a subordinating conjunction
    ('as soon as') — same word, related use, not drafted separately.
---

