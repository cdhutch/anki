---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1277
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.3.4
- pos:preposition
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1277
  Lemma: по́над
  Lemma_Euphony: ''
  PartOfSpeech: preposition
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: over, more than (a quantity)
  Govt_Case: Зн.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: понад
  UA_Example: У опитуванні взяли участь понад тисяча мешканців.
  EN_Example: Over a thousand residents took part in the survey.
  Tags_Ch: ch:2.3.4
  Source_URL: https://goroh.pp.ua/Словозміна/понад
  Source_Note: Горох-verified 2026-08-28, ch.3.4 sourcing pass (village/town governance, heritage
    architecture, public opinion).
  Verification Notes: New. Invariable preposition, typically governs Зн. (accusative) with
    numeral quantities, e.g. понад тисячу. Горох also lists a rare feminine noun понада under
    the same spelling — unrelated, not registered.
---

