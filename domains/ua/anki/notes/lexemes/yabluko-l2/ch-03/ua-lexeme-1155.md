---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1155
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.3.1
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-1155
  Lemma: ба́нка
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: jar
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: Горох also lists ба́нка "мілина" (a shoal/sandbank) and ба́нка "лавка" (a
    bench/pew, regional/dialectal) under the same spelling and declension; neither registered
    (the container sense is meant here).
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: банка
  UA_Example: Мама купила банку меду.
  EN_Example: Mom bought a jar of honey.
  Tags_Ch: ch:2.3.1
  Source_URL: https://goroh.pp.ua/Словозміна/банка
  Source_Note: Горох-verified 2026-08-28, ch.3.1 sourcing pass (quantities/containers).
  Verification Notes: New. Fem., 1st decl., hard group.
---

