---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1261
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:3.4
- pos:adjective
- status:draft
fields:
  NoteID: ua-lexeme-1261
  Lemma: опи́таний
  Lemma_Euphony: ''
  PartOfSpeech: adjective
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: surveyed, polled (person)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: опитаний
  UA_Example: 82% опитаних підтримують ремонт фасаду.
  EN_Example: 82% of those surveyed support renovating the facade.
  Tags_Ch: ch:3.4
  Source_URL: https://goroh.pp.ua/Словозміна/опитаний
  Source_Note: Горох-verified 2026-08-28, ch.3.4 sourcing pass (village/town governance, heritage
    architecture, public opinion).
  Verification Notes: New. Past passive participle of опитати (pf, “to survey/poll”, Горох-verified),
    used adjectivally/substantivally in survey-report language (і опитаних “of those surveyed”);
    drafted as adjective per corpus convention for participles.
---

