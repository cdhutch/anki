---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2287
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.4.4
- ch:1.12.5
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-2287
  Lemma: влаштува́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to organize, arrange, hold (an event) (perfective)
  Govt_Case: Зн. (влаштувати щось)
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: влаштувати
  UA_Example: Школа влаштувала змагання минулого тижня.
  EN_Example: The school held a competition last week.
  Tags_Ch: ch:2.4.4, ch:1.12.5
  Source_URL: https://goroh.pp.ua/Словозміна/влаштувати
  Source_Note: Горох-verified 2026-08-29, aspect-pairing completeness pass (fixing incomplete
    verb aspect pairs flagged by count-incomplete-aspect-pairs audit), cross-checked against
    yabluko-l2-verb-dictionary.pdf for every verb's aspect partner per Craig's explicit instruction.
  Verification Notes: New. Perfective partner of влашто́вувати (ua-lexeme-1462), per yabluko-l2-verb-dictionary.pdf.
    Drafted during the aspect-pairing completeness pass (Craig's request).
---

