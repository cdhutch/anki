---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1508
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.4.5
- ch:2.6.1
- ch:1.11.1
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1508
  Lemma: дава́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to give
  Govt_Case: Зн., Д.
  IrregularForms: ''
  CounterpartForm: да́ти (ua-lexeme-1651, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: давати
  UA_Example: Вона завжди давала слушні поради.
  EN_Example: She always gave sound advice.
  Tags_Ch: ch:2.4.5, ch:2.6.1, ch:1.11.1
  Source_URL: https://goroh.pp.ua/Словозміна/давати
  Source_Note: Горох-verified 2026-08-28, ch.4.5 sourcing pass (upbringing/community/generations;
    character traits).
  Verification Notes: New. Imperfective. Textbook wordlist targets давати собі раду (this
    sub-chapter, ua-lexeme-1484).
---

