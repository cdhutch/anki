---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1366
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.4.1
- ch:1.11.2
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-1366
  Lemma: століття
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: n
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: century
  Govt_Case: ''
  IrregularForms: 'Gen. pl. shortened: століть (not the expected longer form).'
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: століття
  UA_Example: Ця церква була збудована у пʼятнадцятому столітті.
  EN_Example: This church was built in the fifteenth century.
  Tags_Ch: ch:2.4.1, ch:1.11.2
  Source_URL: https://goroh.pp.ua/Словозміна/століття
  Source_Note: 'Горох-verified 2026-08-28, ch.4.1 sourcing pass (history: centuries, era markers,
    everyday inventions).'
  Verification Notes: New. Neut., 2nd decl., soft group. Stress столі́ття (2nd syllable).
---

