---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1373
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.9.7
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.4.1
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-1373
  Lemma: черга
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: queue, line; turn
  Govt_Case: ''
  IrregularForms: 'Gen. pl. shortened: черг.'
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: черга
  UA_Example: Ми стояли у довгій черзі до музею.
  EN_Example: We stood in a long line for the museum.
  Tags_Ch: ch:2.4.1, ch:1.9.7
  Source_URL: https://goroh.pp.ua/Словозміна/черга
  Source_Note: 'Горох-verified 2026-08-28, ch.4.1 sourcing pass (history: centuries, era markers,
    everyday inventions).'
  Verification Notes: New. Fem., 1st decl., hard group. Stress fixed on 1st syllable, че́рга,
    throughout the paradigm.
---

