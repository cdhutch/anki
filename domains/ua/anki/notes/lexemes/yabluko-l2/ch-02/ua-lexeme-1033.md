---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1033
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.3
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1033
  Lemma: ліпи́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: зліпи́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to mold, sculpt (e.g. clay, dough)
  Govt_Case: Зн., з+Р., Ор.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: ліпити / зліпити
  UA_Example: Діти люблять ліпити фігурки з глини.
  EN_Example: Children love molding figures out of clay.
  Tags_Ch: ch:2.2.3
  Source_URL: https://goroh.pp.ua/Словозміна/ліпити; https://goroh.pp.ua/Словозміна/зліпити
  Source_Note: Горох-verified 2026-08-28, ch.2.3 sourcing pass (crafts/toys; materials).
  Verification Notes: New. imperfective., non-reflexive, 2 conj. Irregular 2nd-conj. verb
    (ліплю́ but лі́пиш... л-epenthesis in 1sg); no reflexive. Governs Зн., з+Р., Ор. per yabluko-l2-verb-dictionary.
    Paired with ua-verb-0204. Perfective зліпи́ти added (yabluko-l2-verb-dictionary primary
    pairing, Горох stress-verified, 2026-08-28, ch.2.3 drafting pass).
---

