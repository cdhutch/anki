---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1116
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.6
- ch:1.12.1
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1116
  Lemma: ра́дити
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: пора́дити
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to advise, recommend
  Govt_Case: Д., Зн.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: радити / порадити
  UA_Example: Бібліотекар завжди радить цікаві книги.
  EN_Example: The librarian always recommends interesting books.
  Tags_Ch: ch:2.2.6, ch:1.12.1
  Source_URL: https://goroh.pp.ua/Словозміна/радити; https://goroh.pp.ua/Словозміна/порадити
  Source_Note: Горох-verified 2026-08-28, ch.2.6 sourcing pass (book genres/plots).
  Verification Notes: New. imperfective., non-reflexive, 2 conj. Regular 2nd-conj. verb; no
    reflexive. Governs Д. (комусь) + Зн./inf. (щось/щось робити) per yabluko-l2-verb-dictionary.
    Paired with ua-verb-0234. Perfective пора́дити added (yabluko-l2-verb-dictionary primary
    pairing, Горох stress-verified, 2026-08-28, ch.2.6 drafting pass).
---

