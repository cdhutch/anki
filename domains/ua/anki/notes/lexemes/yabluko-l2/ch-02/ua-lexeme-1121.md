---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1121
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.7.7
- ch:1.7.6
- ch:1.7.3
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.7
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-1121
  Lemma: зал
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: m
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: hall
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: за́ла (fem., 1st decl.) -- an alternate-gender headword with the same meaning
    per Горох; not registered separately, зал (masc.) is the standard/dominant form.
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: зал
  UA_Example: Виставковий зал був переповнений відвідувачами.
  EN_Example: The exhibition hall was packed with visitors.
  Tags_Ch: ch:2.2.7, ch:1.7.3, ch:1.7.6, ch:1.7.7
  Source_URL: https://goroh.pp.ua/Словозміна/зал
  Source_Note: Горох-verified 2026-08-28, ch.2.7 sourcing pass (invitations/exhibitions, formal
    letters).
  Verification Notes: New. Masc., 2nd decl., hard group.
---

