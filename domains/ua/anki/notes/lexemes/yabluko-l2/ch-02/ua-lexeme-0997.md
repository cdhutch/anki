---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0997
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.7.5
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.2
- pos:adjective
- status:draft
- ch:2.2.3
fields:
  NoteID: ua-lexeme-0997
  Lemma: скляни́й
  Lemma_Euphony: ''
  PartOfSpeech: adjective
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: glass, made of glass
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: скляний
  UA_Example: Не залишай скляний посуд на пляжі.
  EN_Example: Don't leave glass tableware at the beach.
  Tags_Ch: ch:2.2.2, ch:2.2.3, ch:1.7.5
  Source_URL: https://goroh.pp.ua/Словозміна/скляний
  Source_Note: Горох-verified 2026-08-28, ch.2.2 sourcing pass (beach picnic; tableware).
  Verification Notes: New. Hard-group adjective. Example bundles по́суд (ua-lexeme-0995) and
    пляж (ua-lexeme-0998), this chapter. This word recurs in the ch.2.3 materials wordlist
    (глиняний/деревʼяний/металевий/паперовий/полотняний/скляний/шерстяний) -- expect a bucket-3
    dedup retag to ch:2.3 when that sub-chapter is drafted.
---

