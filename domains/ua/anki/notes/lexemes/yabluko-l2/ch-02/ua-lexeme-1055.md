---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1055
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.5
- ch:1.11.6
- ch:1.1.5
- ch:1.4.4
- ch:1.10.1
- pos:adjective
- status:draft
fields:
  NoteID: ua-lexeme-1055
  Lemma: розу́мний
  Lemma_Euphony: ''
  PartOfSpeech: adjective
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: smart, intelligent; reasonable
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: розумний
  UA_Example: Це розумне рішення.
  EN_Example: This is a smart/reasonable decision.
  Tags_Ch: ch:2.2.5, ch:1.11.6, ch:1.1.5, ch:1.4.4, ch:1.10.1
  Source_URL: https://goroh.pp.ua/Словозміна/розумний
  Source_Note: Горох-verified 2026-08-28, ch.2.5 sourcing pass (party etiquette/gestures).
  Verification Notes: New. Hard-group adjective. Example bundles межа (ua-lexeme-1054, this
    chapter) -- wordlist item "розумні межі" (reasonable limits).
---

