---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1058
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.5
- pos:noun
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1058
  Lemma: тост
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: m
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: toast (a drinking toast/speech)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: тост
  UA_Example: Він підняв келих і сказав тост.
  EN_Example: He raised his glass and gave a toast.
  Tags_Ch: ch:2.2.5
  Source_URL: https://goroh.pp.ua/Словозміна/тост
  Source_Note: Горох-verified 2026-08-28, ch.2.5 sourcing pass (party etiquette/gestures).
  Verification Notes: New. Masc., 2nd decl., hard group. Example bundles сказати (ua-lexeme-0317,
    retagged ch:2.5 this pass) and келих (ua-lexeme-1065, this chapter) -- wordlist item "сказати
    тост".
---

