---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1080
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.5
- ch:1.4.3
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1080
  Lemma: цілува́тися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: поцілува́тися
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to kiss (each other)
  Govt_Case: з+Ор., в+Зн.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: цілуватися / поцілуватися
  UA_Example: Гості цілуються в щоку на знак вітання.
  EN_Example: The guests kiss each other on the cheek as a greeting.
  Tags_Ch: ch:2.2.5, ch:1.4.3
  Source_URL: https://goroh.pp.ua/Словозміна/цілуватися; https://goroh.pp.ua/Словозміна/поцілуватися
  Source_Note: Горох-verified 2026-08-28, ch.2.5 sourcing pass (party etiquette/gestures).
  Verification Notes: New. imperfective., reflexive, 1 conj. Regular 1st-conj. reflexive (reciprocal)
    verb. Governs з+Ор. (з кимось) and в+Зн. (в щоку) per yabluko-l2-verb-dictionary. Paired
    with ua-verb-0218. Perfective поцілува́тися added (yabluko-l2-verb-dictionary primary
    pairing, Горох stress-verified, 2026-08-28, ch.2.5 drafting pass).
---

