---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1152
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.7
- pos:phrase
- status:draft
fields:
  NoteID: ua-lexeme-1152
  Lemma: За додатко́вою інформа́цією зверта́йтеся...
  Lemma_Euphony: ''
  PartOfSpeech: phrase
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: For additional information, contact... (formal formula)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: За додатковою інформацією звертайтеся...
  UA_Example: За додатковою інформацією звертайтеся до куратора виставки.
  EN_Example: For additional information, contact the exhibition curator.
  Tags_Ch: ch:2.7
  Source_URL: https://goroh.pp.ua/Словозміна/За додатковою інформацією звертайтеся...
  Source_Note: Горох-verified 2026-08-28, ch.2.7 sourcing pass (invitations/exhibitions, formal
    letters).
  Verification Notes: New. Fixed formal-letter formula. Bundles додатковий (ua-lexeme-1140),
    інформація (ua-lexeme-1142), and звертатися/звернутися (ua-lexeme-1147/ua-verb-0239),
    all this chapter.
---

