---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1117
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.6
- pos:phrase
- status:draft
fields:
  NoteID: ua-lexeme-1117
  Lemma: Що Ви мо́жете пора́дити?
  Lemma_Euphony: ''
  PartOfSpeech: phrase
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: What can you recommend?
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: Що Ви можете порадити?
  UA_Example: Що Ви можете порадити для початківця?
  EN_Example: What can you recommend for a beginner?
  Tags_Ch: ch:2.2.6
  Source_URL: https://goroh.pp.ua/Словозміна/Що Ви можете порадити?
  Source_Note: Горох-verified 2026-08-28, ch.2.6 sourcing pass (book genres/plots).
  Verification Notes: New. Fixed polite-register (Ви) request-for-recommendation formula.
    Bundles порадити (ua-lexeme-1116, this chapter); могти (ua-lexeme-0938, already in the
    corpus, ch:1.6) is a supporting modal verb here, not the chapter's focal new item, so
    not retagged.
---

