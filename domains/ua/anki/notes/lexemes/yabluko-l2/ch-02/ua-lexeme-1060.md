---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1060
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.9.5
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.2.5
- ch:1.10.1
- pos:noun
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1060
  Lemma: голова́
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: head (body part)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: голова́ "той, хто керує зборами; керівник організації" (chairperson, head
    of an organization) -- distinct homograph, istota, same declension pattern; not registered
    this pass (body-part sense meant here).
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: голова
  UA_Example: Вона кивнула головою на знак згоди.
  EN_Example: She nodded her head in agreement.
  Tags_Ch: ch:2.2.5, ch:1.9.5, ch:1.10.1
  Source_URL: https://goroh.pp.ua/Словозміна/голова
  Source_Note: Горох-verified 2026-08-28, ch.2.5 sourcing pass (party etiquette/gestures).
  Verification Notes: New. Fem., 1st decl., hard group. Example bundles кивати/кивнути (ua-lexeme-1074/ua-verb-0212,
    this chapter) -- wordlist item "кивати головою".
---

