---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2176
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.11.5
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-2176
  Lemma: ги́нути
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: заги́нути
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to perish, die (imperfective)
  Govt_Case: від + Р., під час + Р.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: гинути / загинути
  UA_Example: Під час катастроф гинуть тисячі людей.
  EN_Example: During disasters, thousands of people perish.
  Tags_Ch: ch:2.11.5
  Source_URL: https://goroh.pp.ua/Словозміна/гинути
  Source_Note: 'Горох-verified 2026-08-29, ch.11.5 sourcing pass (disaster/crisis vocabulary:
    natural and man-made catastrophes, evacuation/cleanup response, narrative and political-suppression
    terms), cross-checked against yabluko-l2-verb-dictionary.pdf for every verb''s aspect
    partner per Craig''s explicit instruction.'
  Verification Notes: 'New. impf., non-reflexive. Unusual case: гинути is an underived imperfective
    that itself carries the -нути suffix shape normally reserved for perfectives in this corpus''s
    classification (cf. вибухнути, вимкнути) -- classified conj1-нути purely by conjugational
    shape (regular non-past ги́ну/ги́неш..., no vowel/consonant irregularity), while noting
    aspectually it is the imperfective member of the pair, with prefixed загинути as its perfective
    (verb-dictionary line 388: ''гинути загинути від + Р, під час + Р''). Merged duplicate
    standalone perfective note ua-lexeme-2176 (aspect-pairing dual-convention cleanup, 2026-08-29).
    That sub-chapter''s example: ''У землетрусі загинуло багато людей.'' = ''Many people died
    in the earthquake.''.'
---

