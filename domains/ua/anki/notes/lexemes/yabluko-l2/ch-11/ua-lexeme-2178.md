---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2178
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.11.5
- pos:verb
- status:draft
- ch:2.12.5
fields:
  NoteID: ua-lexeme-2178
  Lemma: захища́тися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: захисти́тися
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to protect oneself, defend oneself (imperfective)
  Govt_Case: Ор., від + Р.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: захищатися / захиститися
  UA_Example: Люди намагаються захищатися від радіації.
  EN_Example: People try to protect themselves from radiation.
  Tags_Ch: ch:2.11.5,ch:2.12.5
  Source_URL: https://goroh.pp.ua/Словозміна/захищатися
  Source_Note: 'Горох-verified 2026-08-29, ch.11.5 sourcing pass (disaster/crisis vocabulary:
    natural and man-made catastrophes, evacuation/cleanup response, narrative and political-suppression
    terms), cross-checked against yabluko-l2-verb-dictionary.pdf for every verb''s aspect
    partner per Craig''s explicit instruction.'
  Verification Notes: 'New. impf., reflexive, regular conj1-vowel+й (reflexive of захищати,
    ua-lexeme not separately drafted -- захищатися bulleted directly in the textbook wordlist
    as its own reflexive entry). Perfective захиститися confirmed via yabluko-l2-verb-dictionary.pdf
    (line 621: ''захищатися захиститися Ор., від + Р''). Reflexive -сь colloquial contractions
    from Горох normalized to standard -ся/-теся/-мося throughout (matches established convention,
    cf. відмовитися ch.11.1). Merged duplicate standalone perfective note ua-lexeme-2178 (aspect-pairing
    dual-convention cleanup, 2026-08-29). That sub-chapter''s example: ''Він захистився від
    небезпеки вчасно.'' = ''He protected himself from the danger in time.''.'
---

