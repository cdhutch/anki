---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0495
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.8.3
- pos:phrase
- phrase:discourse
- stress:verified
- status:verified
fields:
  NoteID: ua-lexeme-0495
  Lemma: що стосу́ється
  PartOfSpeech: phrase
  Gender: ''
  Perfective: ''
  EN_Gloss: as regards, concerning, as for
  Govt_Case: ''
  CounterpartForm: ''
  IrregularForms: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  EuphonyNote: ''
  TypingAnswer: що стосується
  UA_Example: Що стосується транспорту, тут усе дуже зручно.
  EN_Example: As for transportation, everything here is very convenient.
  Tags_Ch: ch:2.8.3
  Source_URL: https://goroh.pp.ua/Словозміна/стосуватися
  Source_Note: Горох-verified 2026-07-26, ch.8.3 sourcing pass.
  Verification Notes: Fixed discourse construction built on стосуватися (ua-lexeme-0481, full
    conjugation on ua-verb-0076), which also has its own note per rule 3; що (interrogative/relative
    pronoun) treated as a functional word here and not given a separate note.
  Lemma_Euphony: ''
  Perfective_Euphony: ''
  ImperfectiveUnidirectional_Euphony: ''
  CompareA: ''
  CompareB: ''
  CompareC: ''
  CompareD: ''
  CompareScenario: ''
  Homograph_SenseA: ''
  Homograph_SenseB: ''
  AspectCue: ''
---

