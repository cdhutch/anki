---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0383
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.8.1
- pos:adjective
- stress:verified
- status:verified
fields:
  NoteID: ua-lexeme-0383
  Lemma: ві́тряний
  PartOfSpeech: adjective
  Gender: ''
  Perfective: ''
  EN_Gloss: windy
  Govt_Case: ''
  CounterpartForm: ''
  IrregularForms: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  EuphonyNote: ''
  TypingAnswer: вітряний
  UA_Example: Завтра буде вітряний день.
  EN_Example: Tomorrow will be a windy day.
  Tags_Ch: ch:2.8.1
  Source_URL: https://goroh.pp.ua/Тлумачення/вітряний; https://goroh.pp.ua/Словозміна/вітряний
  Source_Note: Горох-verified 2026-07-26, ch.8.1 sourcing pass.
  Verification Notes: 'Stress-based homograph: ві́тряний (stress on ві-) is Горох''s Тлумачення
    sense "прикметник до ві́тер" (adjective of ві́тер/wind) -- the weather sense used here,
    confirmed via example "Посилення вітру надвечір є ознакою настання вітряної дощової погоди"
    (increasing wind toward evening signals oncoming windy rainy weather). вітряни́й (stress
    on -ни́й) is a distinct sense, "який діє за допомогою вітру" (wind-powered/operated by
    wind, e.g. вітряний млин/windmill) -- not used here, no note created for it pending an
    actual ch.8+ vocabulary hit.'
  Lemma_Euphony: ''
  Perfective_Euphony: ''
  ImperfectiveUnidirectional_Euphony: ''
  CompareA: ''
  CompareB: ''
  CompareC: ''
  CompareD: ''
  CompareScenario: ''
  Homograph_SenseA: ''
  Homograph_SenseB: ''
  AspectCue: ''
---

