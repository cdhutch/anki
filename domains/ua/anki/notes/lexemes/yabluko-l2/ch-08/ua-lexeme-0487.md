---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0487
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.8.3
- pos:phrase
- phrase:noun
- stress:verified
- status:verified
fields:
  NoteID: ua-lexeme-0487
  Lemma: ро́зклад ру́ху
  PartOfSpeech: phrase
  Gender: ''
  Perfective: ''
  EN_Gloss: traffic/transit schedule, timetable
  Govt_Case: ''
  CounterpartForm: ''
  IrregularForms: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  EuphonyNote: ''
  TypingAnswer: розклад руху
  UA_Example: Розклад руху потягів змінився з понеділка.
  EN_Example: The train traffic schedule changed as of Monday.
  Tags_Ch: ch:2.8.3
  Source_URL: https://goroh.pp.ua/Словозміна/розклад; https://goroh.pp.ua/Тлумачення/рух
  Source_Note: Горох-verified 2026-07-26, ch.8.3 sourcing pass.
  Verification Notes: Vocabulary-appendix phrase item; components розклад (ua-lexeme-0462)
    and рух (ua-lexeme-0463) each also have their own note per rule 3.
---

