---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0889
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.1.4
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-0889
  Lemma: заважа́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: зава́дити
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to bother, disturb, hinder
  Govt_Case: Д. (person), Ор. (cause)
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: заважати / завадити
  UA_Example: Не заважай мені, будь ласка, я працюю.
  EN_Example: Please don't bother me, I'm working.
  Tags_Ch: ch:2.1.4
  Source_URL: https://goroh.pp.ua/Словозміна/заважати; https://goroh.pp.ua/Словозміна/завадити
  Source_Note: Горох-verified 2026-08-28, ch.1.4 sourcing pass (mood/dental-care/everyday
    verbs).
  Verification Notes: New. impf., non-reflexive, 1 conj.  Paired with ua-verb-0166. Perfective
    зава́дити added (yabluko-l2-verb-dictionary primary pairing, Горох stress-verified, 2026-08-28,
    ch.1.4 drafting pass).
---

