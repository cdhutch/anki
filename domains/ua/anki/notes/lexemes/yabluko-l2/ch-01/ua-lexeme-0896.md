---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0896
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.1.4
- ch:2.6.1
- pos:verb
- status:draft
- ch:2.7.1
- ch:1.11.2
- ch:1.6.7
fields:
  NoteID: ua-lexeme-0896
  Lemma: чи́стити
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: почи́стити
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to clean, brush
  Govt_Case: Зн., Ор.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: чистити / почистити
  UA_Example: Я чищу зуби перед сном.
  EN_Example: I brush my teeth before bed.
  Tags_Ch: ch:2.1.4, ch:2.6.1, ch:2.7.1, ch:1.11.2, ch:1.6.7
  Source_URL: https://goroh.pp.ua/Словозміна/чистити; https://goroh.pp.ua/Словозміна/почистити
  Source_Note: Горох-verified 2026-08-28, ch.1.4 sourcing pass (mood/dental-care/everyday
    verbs).
  Verification Notes: 'New. impf., non-reflexive, 2 conj. ст/щ mutation in 1sg (чищу), 2nd
    conj. Bundled illustrative phrase "чистити зуби перед сном" used as the example sentence
    here rather than drafted as a separate phrase note: зуб (ua-lexeme-0885) and сон (ua-lexeme-0886)
    are both freshly drafted in this same ch:1.4 pass, not pre-existing gaps -- per the "залишатися
    вдома" precedent (ua-lexeme-0880, ch:1.3). Paired with ua-verb-0173. Perfective почи́стити
    added (yabluko-l2-verb-dictionary primary pairing, Горох stress-verified, 2026-08-28,
    ch.1.4 drafting pass). Merged duplicate standalone perfective note ua-lexeme-0896 (aspect-pairing
    dual-convention cleanup, 2026-08-29). That sub-chapter''s example: ''Почисти, будь ласка,
    взуття перед виходом.'' = ''Please clean your shoes before you go out.''.'
---

