---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0913
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.5
- pos:adverb
- status:draft
fields:
  NoteID: ua-lexeme-0913
  Lemma: неоднозна́чно
  Lemma_Euphony: ''
  PartOfSpeech: adverb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: ambiguously, in a mixed/unclear way
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: неоднозначно
  UA_Example: Це питання сприймається неоднозначно.
  EN_Example: This issue is perceived ambiguously.
  Tags_Ch: ch:1.5
  Source_URL: https://goroh.pp.ua/Словозміна/неоднозначно
  Source_Note: Горох-verified 2026-08-28, ch.1.5 sourcing pass (Christmas customs/traditions;
    celebration verbs).
  Verification Notes: New. Indeclinable adverb, predicative-capable. Example sentence previews
    сприйма́ти (ua-lexeme-0921, ch:1.5, this chapter).
---

