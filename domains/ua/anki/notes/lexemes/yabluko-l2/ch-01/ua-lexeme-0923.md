---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0923
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.1.5
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-0923
  Lemma: стриба́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: стрибну́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to jump
  Govt_Case: М., у/в/на + Зн., до + Р.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: стрибати / стрибнути
  UA_Example: На святі молодь стрибає через вогонь.
  EN_Example: At the festival, young people jump over the fire.
  Tags_Ch: ch:2.1.5
  Source_URL: https://goroh.pp.ua/Словозміна/стрибати; https://goroh.pp.ua/Словозміна/стрибнути
  Source_Note: Горох-verified 2026-08-28, ch.1.5 sourcing pass (Christmas customs/traditions;
    celebration verbs).
  Verification Notes: New. impf., non-reflexive, 1 conj. Example sentence bundles the fixed
    collocation "стрибати через вогонь" -- вогонь (ua-lexeme-0329, ch:2.9.7) already exists
    and is retagged ch:1.5 rather than re-drafted. Paired with ua-verb-0183. Perfective стрибну́ти
    added (yabluko-l2-verb-dictionary primary pairing, Горох stress-verified, 2026-08-28,
    ch.1.5 drafting pass).
---

