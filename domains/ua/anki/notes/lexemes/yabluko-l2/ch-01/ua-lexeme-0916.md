---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0916
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.5
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-0916
  Lemma: облива́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: обли́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to pour (liquid) over, splash, spill on
  Govt_Case: Зн., Ор.
  IrregularForms: ''
  CounterpartForm: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: обливати / облити
  UA_Example: Він випадково облив скатертину соком.
  EN_Example: He accidentally spilled juice on the tablecloth.
  Tags_Ch: ch:1.5
  Source_URL: https://goroh.pp.ua/Словозміна/обливати; https://goroh.pp.ua/Словозміна/облити
  Source_Note: Горох-verified 2026-08-28, ch.1.5 sourcing pass (Christmas customs/traditions;
    celebration verbs).
  Verification Notes: New. impf., non-reflexive, 1 conj.  Paired with ua-verb-0176. Perfective
    обли́ти added (yabluko-l2-verb-dictionary primary pairing, Горох stress-verified, 2026-08-28,
    ch.1.5 drafting pass).
---

