---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0584
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.9.4
- pos:verb
- motion:base
- stress:verified
- status:draft
fields:
  NoteID: ua-lexeme-0584
  Lemma: пла́вати
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: пливти́
  Perfective: попливти́
  EN_Gloss: to swim
  Govt_Case: ''
  CounterpartForm: ''
  IrregularForms: ''
  VerbMotion_Pair: 'Base motion-verb triplet: пла́вати (multidirectional impf.) / пливти́
    (unidirectional impf.) / попливти́ (perf.). Full conjugation tables exist as separate
    UA_Verb notes: ua-verb-0008 (плавати), ua-verb-0009 (пливти), ua-verb-0010 (попливти).'
  ConfusableSet: ''
  Mnemonic_EN: пла́вати = "swims habitually / knows how to swim, no set direction" (multidirectional
    -- e.g. swims around, can swim). пливти́ = "is swimming, headed one way, right now" (unidirectional
    -- e.g. swimming across/toward something). попливти́ = "set off swimming" (perfective
    -- the swim as a completed whole).
  CrossLang_Analog: English "swim" covers both плавати and пливти without marking the habitual-vs-
    single-swim distinction -- closest analogy is "used to swim around here" (multi) vs "was
    swimming toward shore" (uni) vs "set off swimming" (perf).
  EuphonyNote: 'пливти́ and плисти́ are free variants of the same unidirectional imperfective
    "to swim" verb -- both Горох-verified as valid headwords (недоконаний вид, нерефлексивне,
    1 дієвідміна for both). Paradigms are near-identical: present tense (пливу́/пливе́ш/.../пливу́ть),
    imperative (пливи́/пливі́ть), and masc. past (плив) are IDENTICAL for both forms; they
    diverge only in the fem./neut./plural past (пливла́/пливло́/пливли́ vs плила́/плило́/плили́)
    and the synthetic future stem (пливти́му vs плисти́му). пливти́ is used here, matching
    the corpus''s existing UA_Verb conjugation note (ua-verb-0009, headworded пливти). Treat
    the two as free variants of one lexeme, not separate words -- parallels the йти/іти alternation
    documented on ua-lexeme-0581 (ходити group).'
  TypingAnswer: плавати / пливти / попливти
  UA_Example: Діти люблять плавати в озері, а вчора один хлопчик плив до берега.
  EN_Example: Children love to swim in the lake, and yesterday one boy was swimming toward
    the shore.
  Tags_Ch: ch:2.9.4
  Source_URL: https://goroh.pp.ua/Словозміна/плавати; https://goroh.pp.ua/Словозміна/пливти;
    https://goroh.pp.ua/Словозміна/плисти; https://goroh.pp.ua/Словозміна/попливти
  Source_Note: Горох-verified 2026-07-31, base motion-verb triplet drafting session (ua-lexeme-0581..0585).
  Verification Notes: 'Stress marks for all three aspect slots confirmed via Горох Словозміна:
    пла́вати (multi, impf.), пливти́ (uni, impf.), попливти́ (perf.) -- no homograph ambiguity
    on any of the three pages. New note; no prior lexeme entry existed for this base (unprefixed)
    triplet -- only the separate UA_Verb conjugation notes (ua-verb-0008..0010). ConfusableSet
    intentionally left blank -- this is an aspect triplet, not a lexical-confusable/homograph
    pair, so the Compare card is expected to stay suspended. 2026-07-31, per Craig''s catch:
    checked плисти́ against пливти́ on Горох -- confirmed both are valid headwords for the
    same unidirectional-imperfective "to swim" verb (identical present tense and imperative,
    identical masc. past; differ only in fem./neut./plural past and the future stem). Documented
    as a free-variant pair via EuphonyNote, parallel to the йти/іти treatment on ua-lexeme-0581
    -- пливти́ kept as ImperfectiveUnidirectional since it matches the existing ua-verb-0009
    headword. Drafted per Craig''s request 2026-07-31. status:draft pending Craig''s independent
    review.'
---

