---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-0336
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.9.7
- pos:noun
- gender:m
- stress:unverified
- status:draft
fields:
  NoteID: ua-lexeme-0336
  Lemma: поворо́т
  PartOfSpeech: noun
  Gender: m
  Perfective: ''
  EN_Gloss: turn (in road); turning point, shift
  Govt_Case: ''
  CounterpartForm: ''
  IrregularForms: ''
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  EuphonyNote: ''
  TypingAnswer: поворот
  UA_Example: "Одразу після повороту видно було сироварню.\nТакий різкий поворот в економіці\
    \ змінив все.\n"
  EN_Example: "Right after the turn, the cheese dairy came into view.\nSuch an abrupt turning\
    \ point in the economy changed everything.\n"
  Verb_Conj_Table: ''
  Tags_Ch: ch:2.9.7
  Source_URL: https://goroh.pp.ua/Словозміна/поворот
  Source_Note: Горох-verified 2026-07-26, ch:2.9.7 sourcing pass.
  Verification Notes: 'Two primary senses: (1) physical/spatial — a point where a road or
    path changes direction; (2) metaphorical/abstract — a complete change in direction or
    development of something, a turning point or shift in circumstances. Both are common in
    Ukrainian; examples provided to show semantic range.'
---

