...
fields:
  ...
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: леті́ти
  Perfective: полеті́ти
  EN_Gloss: to fly
  Govt_Case: ''
  CounterpartForm: ''
  IrregularForms: ''
  VerbMotion_Pair: 'Base motion-verb triplet: літа́ти (multidirectional impf.) / леті́ти (unidirectional
    impf.) / полеті́ти (perf.). Full conjugation tables exist as separate UA_Verb notes: ua-verb-0014
    (літати), ua-verb-0015 (летіти), ua-verb-0016 (полетіти).'
  ConfusableSet: ''
  Mnemonic_EN: літа́ти = "flies habitually, no set direction" (multidirectional -- e.g. birds
  ...
  Verification Notes: 'Stress marks for all three aspect slots confirmed via Горох Словозміна:
    літа́ти (multi, impf.), леті́ти (uni, impf.), полеті́ти (perf.) -- no homograph ambiguity
