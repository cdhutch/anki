...
fields:
  ...
  Source_Note: Created 2026-07-28 as the same-Lemma polysemy-split sibling to ua-lexeme-0304
    (відправлення, "departure"), per the established pattern (see ua-lexeme-0211/0377 for
    вболівати) -- separate same-Lemma notes with example-driven front-card disambiguation
    rather than one note with a combined multi-sense EN_Gloss. Covers Горох's Тлумачення sense
    4, "Предмет, що пересилається поштою (бандероль, лист, посилка і т. ін.)" (a mailed item
    -- parcel, letter, package) -- ua-lexeme-0304 covers sense 1, the departure/dispatch sense.
    UA_Example is an original composition (not textbook-sourced), modeled on Горох's own citation
