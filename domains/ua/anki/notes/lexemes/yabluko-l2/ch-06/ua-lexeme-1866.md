---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1866
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.7
- ch:1.11.2
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1866
  Lemma: ся́яти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to shine, sparkle (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: зася́яти (ua-lexeme-1867, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: сяяти
  UA_Example: Сонце сяяло крізь листя дерев.
  EN_Example: The sun shone through the leaves of the trees.
  Tags_Ch: ch:2.6.7, ch:1.11.2
  Source_URL: https://goroh.pp.ua/Словозміна/сяяти
  Source_Note: Горох-verified 2026-08-28, ch.6.7 sourcing pass (bear/berry-picking anecdote;
    imperfective-perfective verb pairs; augmentative adjectives).
  Verification Notes: New. Imperfective, "to shine, sparkle". Textbook aspect pair сяяти-засяяти,
    this sub-chapter.
---

