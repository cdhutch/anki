---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1720
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.2
- ch:1.11.6
- ch:1.6.5
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1720
  Lemma: прокида́тися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to wake up (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: проки́нутися (ua-lexeme-1721, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: прокидатися
  UA_Example: Я прокидаюся рано щодня.
  EN_Example: I wake up early every day.
  Tags_Ch: ch:2.6.2, ch:1.11.6, ch:1.6.5
  Source_URL: https://goroh.pp.ua/Словозміна/прокидатися
  Source_Note: Горох-verified 2026-08-28, ch.6.2 sourcing pass (daily routine; imperfective-perfective
    verb pairs; frequency adverbs).
  Verification Notes: New. Imperfective reflexive, "to wake up". Textbook aspect pair прокидатися-прокинутися,
    this sub-chapter.
---

