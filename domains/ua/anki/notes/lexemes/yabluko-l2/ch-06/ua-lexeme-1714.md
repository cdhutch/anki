---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1714
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.2
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1714
  Lemma: поверну́тися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to return, come back (perfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: поверта́тися (ua-lexeme-0611, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: повернутися
  UA_Example: Він повернеться з відрядження у пʼятницю.
  EN_Example: He'll be back from his business trip on Friday.
  Tags_Ch: ch:2.6.2
  Source_URL: https://goroh.pp.ua/Словозміна/повернутися
  Source_Note: Горох-verified 2026-08-28, ch.6.2 sourcing pass (daily routine; imperfective-perfective
    verb pairs; frequency adverbs).
  Verification Notes: New. Perfective partner of поверта́тися (ua-lexeme-0611, whose Perfective
    field already cross-references поверну́тися). Textbook aspect pair повертатися-повернутися,
    this sub-chapter.
---

