---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1706
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.9.5
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.2
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1706
  Lemma: зміни́тися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to change (intr.) (perfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: змі́нюватися (ua-lexeme-1705, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: змінитися
  UA_Example: Він дуже змінився за останній рік.
  EN_Example: He's changed a lot over the past year.
  Tags_Ch: ch:2.6.2, ch:1.9.5
  Source_URL: https://goroh.pp.ua/Словозміна/змінитися
  Source_Note: Горох-verified 2026-08-28, ch.6.2 sourcing pass (daily routine; imperfective-perfective
    verb pairs; frequency adverbs).
  Verification Notes: New. Perfective partner of змі́нюватися (ua-lexeme-1705).
---

