---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1681
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.1
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1681
  Lemma: почи́стити
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to clean (perfective)
  Govt_Case: Зн.
  IrregularForms: ''
  CounterpartForm: чи́стити (ua-lexeme-0896, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: почистити
  UA_Example: Почисти, будь ласка, взуття перед виходом.
  EN_Example: Please clean your shoes before you go out.
  Tags_Ch: ch:2.6.1
  Source_URL: https://goroh.pp.ua/Словозміна/почистити
  Source_Note: Горох-verified 2026-08-28, ch.6.1 sourcing pass (household mishaps; imperfective-perfective
    verb pairs; frequency-adverb phrases).
  Verification Notes: New. Perfective partner of чи́стити (ua-lexeme-0896, whose Perfective
    field already cross-references почи́стити). Textbook aspect pair чистити-почистити, this
    sub-chapter.
---

