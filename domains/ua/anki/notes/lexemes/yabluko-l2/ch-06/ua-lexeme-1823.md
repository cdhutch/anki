---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1823
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.8.5
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.5
- ch:1.11.6
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1823
  Lemma: прино́сити
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to bring (repeatedly / habitually) (imperfective)
  Govt_Case: Зн.
  IrregularForms: ''
  CounterpartForm: принести́ (ua-lexeme-1824, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: приносити
  UA_Example: Поштарка щоранку приносить нам листи.
  EN_Example: The postwoman brings us letters every morning.
  Tags_Ch: ch:2.6.5, ch:1.11.6, ch:1.8.5
  Source_URL: https://goroh.pp.ua/Словозміна/приносити
  Source_Note: Горох-verified 2026-08-28, ch.6.5 sourcing pass (crime/danger anecdote; imperfective-perfective
    verb pairs).
  Verification Notes: New. Imperfective, "to bring". conj2-ity regular fixed-stress subtype,
    с->ш 1sg mutation. Textbook aspect pair приносити-принести, this sub-chapter.
---

