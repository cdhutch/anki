---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1709
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.2
- ch:1.11.1
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1709
  Lemma: ляга́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to lie down (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: лягти́ (ua-lexeme-1710, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: лягати
  UA_Example: Я лягаю спати о десятій вечора.
  EN_Example: I go to bed at ten in the evening.
  Tags_Ch: ch:2.6.2, ch:1.11.1
  Source_URL: https://goroh.pp.ua/Словозміна/лягати
  Source_Note: Горох-verified 2026-08-28, ch.6.2 sourcing pass (daily routine; imperfective-perfective
    verb pairs; frequency adverbs).
  Verification Notes: New. Imperfective, "to lie down". Textbook aspect pair лягати-лягти,
    this sub-chapter.
---

