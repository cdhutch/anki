---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1717
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.2
- ch:1.3.1
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1717
  Lemma: працю́вати
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to work (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: попрацюва́ти (ua-lexeme-1718, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: працювати
  UA_Example: Він працює в лікарні медбратом.
  EN_Example: He works at a hospital as a nurse.
  Tags_Ch: ch:2.6.2, ch:1.3.1
  Source_URL: https://goroh.pp.ua/Словозміна/працювати
  Source_Note: Горох-verified 2026-08-28, ch.6.2 sourcing pass (daily routine; imperfective-perfective
    verb pairs; frequency adverbs).
  Verification Notes: New lexeme note for the pre-existing verb-only note ua-verb-0297 (ch:2.4.4),
    now independently bulleted here as the працювати-попрацювати aspect pair.
---

