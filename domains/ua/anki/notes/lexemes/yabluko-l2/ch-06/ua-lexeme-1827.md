---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1827
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.5
- ch:1.11.3
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1827
  Lemma: вря́тувати
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to save, rescue (perfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: ряту́вати (ua-lexeme-1826, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: врятувати
  UA_Example: Пожежники врятували всіх мешканців будинку.
  EN_Example: The firefighters saved all the building's residents.
  Tags_Ch: ch:2.6.5, ch:1.11.3
  Source_URL: https://goroh.pp.ua/Словозміна/врятувати
  Source_Note: Горох-verified 2026-08-28, ch.6.5 sourcing pass (crime/danger anecdote; imperfective-perfective
    verb pairs).
  Verification Notes: New. Perfective partner of ряту́вати (ua-lexeme-1826).
---

