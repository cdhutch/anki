---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1671
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.1
- ch:1.11.2
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1671
  Lemma: пра́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to wash, launder (clothes) (imperfective)
  Govt_Case: Зн.
  IrregularForms: Present stem пер- (vowel-grade alternation from the пра- infinitive/past
    stem, same ablaut type as бра́ти/беру́).
  CounterpartForm: ви́прати (ua-lexeme-1672, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: прати
  UA_Example: Мама пере білизну щосуботи.
  EN_Example: Mom does the laundry every Saturday.
  Tags_Ch: ch:2.6.1, ch:1.11.2
  Source_URL: https://goroh.pp.ua/Словозміна/прати
  Source_Note: Горох-verified 2026-08-28, ch.6.1 sourcing pass (household mishaps; imperfective-perfective
    verb pairs; frequency-adverb phrases).
  Verification Notes: New. Imperfective, "to wash, launder (clothes)". Textbook aspect pair
    прати-випрати, this sub-chapter. Merged duplicate note(s) ua-lexeme-2341 into this canonical
    note during corpus-wide dedup audit (2026-08-29); their chapter tags were migrated here.
---

