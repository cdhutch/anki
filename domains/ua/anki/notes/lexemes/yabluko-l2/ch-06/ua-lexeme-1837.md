---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1837
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.6
- pos:verb
- status:draft
- ch:2.11.5
fields:
  NoteID: ua-lexeme-1837
  Lemma: скасува́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to cancel (perfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: скасо́вувати (ua-lexeme-1836, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: скасувати
  UA_Example: Рейс скасували через негоду.
  EN_Example: The flight was cancelled because of bad weather.
  Tags_Ch: ch:2.6.6,ch:2.11.5
  Source_URL: https://goroh.pp.ua/Словозміна/скасувати
  Source_Note: Горох-verified 2026-08-28, ch.6.6 sourcing pass (UFO anecdote; storytelling
    discourse formulas).
  Verification Notes: New. Perfective partner of скасо́вувати (ua-lexeme-1836). Confirmed
    via Горох.
---

