---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1836
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.6
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1836
  Lemma: скасо́вувати
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to cancel (imperfective)
  Govt_Case: Зн.
  IrregularForms: ''
  CounterpartForm: скасува́ти (ua-lexeme-1837, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: скасовувати
  UA_Example: Організатори не люблять скасовувати заходи в останню хвилину.
  EN_Example: The organizers don't like cancelling events at the last minute.
  Tags_Ch: ch:2.6.6
  Source_URL: https://goroh.pp.ua/Словозміна/скасовувати
  Source_Note: Горох-verified 2026-08-28, ch.6.6 sourcing pass (UFO anecdote; storytelling
    discourse formulas).
  Verification Notes: New. Imperfective, "to cancel". Textbook aspect pair скасовувати-скасувати,
    this sub-chapter.
---

