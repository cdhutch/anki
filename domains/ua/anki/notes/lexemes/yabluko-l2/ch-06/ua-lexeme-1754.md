---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1754
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.3
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1754
  Lemma: довести́
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to prove; to bring/lead (to a state) (perfective)
  Govt_Case: ''
  IrregularForms: 'Non-past stem доведу-/довед- (-сти infinitive, вести-family: веду́, веде́ш);
    irregular masc. past дові́в (vs. regular довела́/довело́/довели́).'
  CounterpartForm: дово́дити (ua-lexeme-1753, imperfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: довести
  UA_Example: Вона довела свою тезу на практиці.
  EN_Example: She proved her thesis in practice.
  Tags_Ch: ch:2.6.3
  Source_URL: https://goroh.pp.ua/Словозміна/довести
  Source_Note: Горох-verified 2026-08-28, ch.6.3 sourcing pass (crime/prison; getting lost;
    solitude; imperfective-perfective verb pairs; duration/manner adverbs).
  Verification Notes: New. Perfective partner of дово́дити (ua-lexeme-1753).
---

