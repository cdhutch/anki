---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1723
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.2
- ch:1.11.2
- pos:verb
- status:draft
fields:
  NoteID: ua-lexeme-1723
  Lemma: слу́хати
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to listen (imperfective)
  Govt_Case: Зн.
  IrregularForms: ''
  CounterpartForm: послу́хати (ua-lexeme-0773, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: слухати
  UA_Example: Він любить слухати музику ввечері.
  EN_Example: He likes listening to music in the evening.
  Tags_Ch: ch:2.6.2, ch:1.11.2
  Source_URL: https://goroh.pp.ua/Словозміна/слухати
  Source_Note: Горох-verified 2026-08-28, ch.6.2 sourcing pass (daily routine; imperfective-perfective
    verb pairs; frequency adverbs).
  Verification Notes: New. Imperfective partner of послу́хати (ua-lexeme-0773, ch:2.10, drafted
    there without an imperfective cross-reference). Textbook aspect pair слухати-послухати,
    this sub-chapter.
---

