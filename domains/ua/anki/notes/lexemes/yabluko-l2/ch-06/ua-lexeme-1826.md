---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-1826
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:2.6.5
- ch:1.11.3
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-1826
  Lemma: ряту́вати
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: ''
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to save, rescue (imperfective)
  Govt_Case: Зн.
  IrregularForms: ''
  CounterpartForm: вря́тувати (ua-lexeme-1827, perfective)
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: рятувати
  UA_Example: Лікарі рятують життя щодня.
  EN_Example: Doctors save lives every day.
  Tags_Ch: ch:2.6.5, ch:1.11.3
  Source_URL: https://goroh.pp.ua/Словозміна/рятувати
  Source_Note: Горох-verified 2026-08-28, ch.6.5 sourcing pass (crime/danger anecdote; imperfective-perfective
    verb pairs).
  Verification Notes: New. Imperfective, "to save, rescue". Textbook aspect pair рятувати-врятувати,
    this sub-chapter.
---

