---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-3226
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.6.6
- ch:1.7.1
- pos:noun
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-3226
  Lemma: сто́лик
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: m
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: null
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: (restaurant/café) table
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: null
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: столик
  UA_Example: Це столик.
  EN_Example: This is a table.
  Tags_Ch: ch:1.6.6, ch:1.7.1
  Source_URL: https://goroh.pp.ua/Словозміна/столик
  Source_Note: yabluko-l1-vocabulary.pdf ch.6, yabluko-l1 vocabulary expansion pass, 2026-08-29.
  Verification Notes: New (yabluko-l1 vocabulary expansion pass, 2026-08-29). Merged duplicate
    ua-lexeme-3278 into this note (final corpus-wide dual-convention-duplicate audit, ch.12
    pass, 2026-08-29); both notes were drafted within this same vocabulary-expansion pass
    with the same meaning -- ua-lexeme-3278's chapter tags migrated here.
---

