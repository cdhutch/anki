---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-3063
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.6.2
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.5.5
- pos:noun
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-3063
  Lemma: курага́
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: f
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: null
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: dried apricots (collective, singular-only)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: null
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: курага
  UA_Example: 'Це популярна страва: курага.'
  EN_Example: 'This is a popular dish: dried apricots.'
  Tags_Ch: ch:1.5.5, ch:1.6.2
  Source_URL: https://goroh.pp.ua/Словозміна/курага
  Source_Note: yabluko-l1-vocabulary.pdf ch.5, yabluko-l1 vocabulary expansion pass, 2026-08-29.
  Verification Notes: New (yabluko-l1 vocabulary expansion pass, 2026-08-29). Merged duplicate
    note(s) ua-lexeme-3150 into this canonical note during corpus-wide dedup audit (2026-08-29);
    their chapter tags were migrated here.
---

