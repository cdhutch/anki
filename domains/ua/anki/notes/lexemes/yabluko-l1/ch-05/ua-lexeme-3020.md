---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-3020
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- ch:1.6.1
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.5.3
- pos:noun
- status:draft
fields:
  NoteID: ua-lexeme-3020
  Lemma: компо́т
  Lemma_Euphony: ''
  PartOfSpeech: noun
  Gender: m
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: null
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: kompot (Ukrainian stewed-fruit drink)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: null
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: компот
  UA_Example: Ми замовили компот.
  EN_Example: We ordered kompot.
  Tags_Ch: ch:1.5.3, ch:1.6.1
  Source_URL: https://goroh.pp.ua/Словозміна/компот
  Source_Note: yabluko-l1-vocabulary.pdf ch.5, yabluko-l1 vocabulary expansion pass, 2026-08-29.
  Verification Notes: New (yabluko-l1 vocabulary expansion pass, 2026-08-29). Merged duplicate
    note(s) ua-lexeme-3135 into this canonical note during corpus-wide dedup audit (2026-08-29);
    their chapter tags were migrated here.
---

