---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2297
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.11.1
- ch:1.2.6
- ch:1.3.4
- ch:1.12.6
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-2297
  Lemma: дивитися
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: подивитися
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to look, watch (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: null
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: дивитися / подивитися
  UA_Example: Діти дивляться мультфільм.
  EN_Example: The children are watching a cartoon.
  Tags_Ch: ch:1.11.1, ch:1.2.6, ch:1.3.4, ch:1.12.6
  Source_URL: https://goroh.pp.ua/Словозміна/дивитися
  Source_Note: yabluko-l1-vocabulary.pdf ch.11.1, drafted from the textbook's own inline conjugation-class
    annotation (conj2-ity); yabluko-l1 vocabulary expansion pass, 2026-08-29.
  Verification Notes: New (yabluko-l1 vocabulary expansion pass, 2026-08-29). Single combined
    note; aspect pair drawn from yabluko-l1-vocabulary.pdf ch.11.1.
---

