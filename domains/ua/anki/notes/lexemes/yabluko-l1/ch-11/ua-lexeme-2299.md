---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2299
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.11.1
- ch:1.3.2
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-2299
  Lemma: запи́тувати
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: ''
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: запита́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to ask (a question) (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: null
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: запитувати / запитати
  UA_Example: Учень запитує вчителя про домашнє завдання.
  EN_Example: The student asks the teacher about the homework.
  Tags_Ch: ch:1.11.1, ch:1.3.2
  Source_URL: https://goroh.pp.ua/Словозміна/запитувати
  Source_Note: yabluko-l1-vocabulary.pdf ch.11.1, drafted from the textbook's own inline conjugation-class
    annotation (conj1-vowel+й); yabluko-l1 vocabulary expansion pass, 2026-08-29.
  Verification Notes: New (yabluko-l1 vocabulary expansion pass, 2026-08-29). Single combined
    note; aspect pair drawn from yabluko-l1-vocabulary.pdf ch.11.1.
---

