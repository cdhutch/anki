---
schema: cnsf/v0
note_type: ua_lexeme
note_id: ua-lexeme-2396
anki:
  model: UA_Lexeme
  deck: UA::Recognition::UA→EN
tags:
- domain:ua
- topic:vocabulary
- textbook:яблуко
- ch:1.11.3
- pos:verb
- status:draft
- release:pending
fields:
  NoteID: ua-lexeme-2396
  Lemma: зустріча́ти
  Lemma_Euphony: ''
  PartOfSpeech: verb
  Gender: null
  ImperfectiveUnidirectional: ''
  ImperfectiveUnidirectional_Euphony: ''
  Perfective: зустрі́ти
  Perfective_Euphony: ''
  EuphonyNote: ''
  AspectCue: ''
  EN_Gloss: to meet, encounter (someone) (imperfective)
  Govt_Case: ''
  IrregularForms: ''
  CounterpartForm: null
  VerbMotion_Pair: ''
  ConfusableSet: ''
  Mnemonic_EN: ''
  CrossLang_Analog: ''
  TypingAnswer: зустрічати / зустріти
  UA_Example: Я зустрічаю друзів у кафе.
  EN_Example: I meet friends at the cafe.
  Tags_Ch: ch:1.11.3
  Source_URL: https://goroh.pp.ua/Словозміна/зустрічати
  Source_Note: yabluko-l1-vocabulary.pdf ch.11.3, yabluko-l1 vocabulary expansion pass, 2026-08-29.
  Verification Notes: New (yabluko-l1 vocabulary expansion pass, 2026-08-29). Single combined
    note; conjugation class per wordlist (conj1-vowel+й).
---

